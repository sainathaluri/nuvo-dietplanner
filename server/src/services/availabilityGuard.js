...
export async function assertSlotAvailable({ dietitianId, scheduledAt, excludeCallId }, conn) {
  ...
  // Sequential, not Promise.all: these three queries share one connection (it must be the
  // transaction's own connection, see above), and every other multi-query model in this codebase
  // (e.g. Plan.js) runs a connection's queries one at a time rather than concurrently.
...
export async function getAvailableSlotsForDay({ dietitianId, date, excludeCallId }, conn = pool) {
  ...
  // Widen the fetch by one slot-duration on each side so a call that starts just before midnight
  // (and so overlaps this day's first candidate slot) or just after this day ends isn't missed —
  // listAvailableSlots's per-candidate overlap check still only counts a call that actually
  // overlaps a given candidate, so the extra rows are harmless.
  ...
  return listAvailableSlots({ workingHours, exceptions, existingCalls, date, durationMinutes: CALL_DURATION_MINUTES });
}
