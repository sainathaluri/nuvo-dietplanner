...
// { activeOnly? } — the create-client dropdowns only want active plans; the admin management
// screen wants every plan, active or not, so it can still list (and reactivate) an inactive one.
export async function listProgramPlans({ activeOnly = false } = {}) {
  const whereSql = activeOnly ? ' WHERE active = TRUE' : '';
  const [rows] = await pool.query(`SELECT * FROM program_plans${whereSql} ORDER BY name ASC`);
...
export async function createProgramPlan({ name, description = null, active = true }) {
  const id = newId();
  await pool.query('INSERT INTO program_plans (id, name, description, active) VALUES (?, ?, ?, ?)', [
    id,
    name,
    description,
    active,
