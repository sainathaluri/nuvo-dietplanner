...
import { getTodayHighlightMeal, getNextCall, computeProgressStats } from '@/lib/clientPortal';
...
export function ClientOverviewScreen() {
  ...
  const highlightMeal = getTodayHighlightMeal(planQuery.plan);
      ...
      <div className="grid gap-5 min-[900px]:grid-cols-2">
        <section className="rounded-card bg-white p-6 shadow-soft">
          ...
          {planQuery.isLoading ? (
            ...
            <div className="mt-4">
              <EmptyState title="No meals planned for today" description="Your dietitian hasn't added meals for today yet." />
            </div>
          )}
          ...
          <div className="mt-5 grid grid-cols-2 gap-3">
            <StatCard
              ...
              hint={planQuery.plan ? `Week of ${formatDate(planQuery.plan.week)}` : undefined}
            />
            <StatCard
              label="Latest weight"
              tone="sage"
